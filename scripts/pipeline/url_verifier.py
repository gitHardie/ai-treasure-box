...
# Known product name -> official homepage mapping
KNOWN_PRODUCTS = {
...
class URLVerifier:
    ...
    def _guess_official_domain(self, name):
        """Try to guess the official domain by HEAD requests to common TLDs."""
        if not name or len(name) > 30:
            return None
        brand = re.sub(r'\d+(\.\d+)*', '', name)
        brand = re.sub(r'\b(family|pro|max|plus|beta|alpha|sdk|api|code|lite|mini)\b', '', brand, flags=re.IGNORECASE)
        words = brand.strip().split()
        if not words:
            return None
        candidate = words[0].lower()
        if len(candidate) <= 2 and len(words) > 1:
            candidate = words[1].lower()
        elif len(candidate) <= 2:
            return None
        generic = {'the', 'ai', 'new', 'my', 'auto', 'smart', 'open', 'web', 'app', 'for', 'and', 'with'}
        if candidate in generic:
            return None
        tlds = ['.ai', '.com', '.io', '.app', '.dev']
        try:
            import requests as req
            session = req.Session()
            session.headers.update({'User-Agent': 'Mozilla/5.0 (compatible; AITreasureBox/1.0)'})
            for tld in tlds:
                test_url = f'https://{candidate}{tld}'
                try:
    ...
    @staticmethod
    def _normalize_known_url(name, url):
        """Normalize URLs for known products that commonly resolve to deep pages."""
        ...
        try:
            ...
            return url
        for product_key, homepage in KNOWN_PRODUCTS.items():
            if product_key in name_lower:
                homepage_domain = urlparse(homepage).netloc.lower()
                if homepage_domain in domain or domain in homepage_domain:
                    return homepage
        google_products = {
            'gemini': 'https://gemini.google.com',
            'aistudio': 'https://aistudio.google.com',
        ...
        for keyword, correct_url in google_products.items():
            if keyword in name_lower and 'google' in domain:
                correct_domain = urlparse(correct_url).netloc.lower()
                if domain != correct_domain:
                    return correct_url
        if 'maps.google' in domain or 'm.google' in domain:
            return url
        return url
    ...
    def _resolve_official_url(self, tool):
        ...
        if "aiww.com" in domain:
            ...
            if result:
                result = self._normalize_known_url(name, result)
                return result
        ...
        for result in top_results:
            ...
            if href and not self._is_aggregator_url(href) and not self._is_false_positive(href):
                href = self._normalize_known_url(name, href)
        ...
        # ---- Strategy 3: Domain guessing for well-known brands ----------
        guessed = self._guess_official_domain(name)
        if guessed:
            guessed = self._normalize_known_url(name, guessed)
            logger.info("[URLVerifier] %s -> guessed domain: %s", name, guessed)
            return guessed
